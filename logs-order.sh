...
for i in "${dirs[@]}"; do
		if mkdir -p $dockDir/logs/"$i"/"$today" && mkdir -p $dockDir/backups/"$i"/"$today"; then
				printf "Folders %s/logs/%s/%s and $dockDir/backups/$i/$today \
...
for i in "${dirs[@]}"; do
	if aws s3 mv $dockDir/logs/"$i"/"$today" s3://slunarcrossover/logs/"$i"/"$today" \
	"${awsOptsLogs[@]}" && aws s3 mv $dockDir/backups/"$i"/"$today" s3://slunarcrossover/backups/"$i"/"$today" \
	"${awsOptsBcks[@]}"; then
